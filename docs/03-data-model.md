...
At startup `RedisServiceImpl::Init` builds `databases` (config key `local.databases`, default 16) prebuilt tables named `data_table_0` .. `data_table_15`, each a `TableName{name, TableType::Primary, TableEngine::EloqKv}`, plus two system tables: `__ns_0` (namespace registry) and `ns_data_0` (shared data table for all non-default namespaces) — `src/redis_service.cpp:232-281`. All are handed to `DataSubstrate::RegisterEngine(TableEngine::EloqKv, &catalog_factory, ...)` (`src/redis_service.cpp:315`), which seeds the engine catalog so no DDL ever runs for Redis databases.
...
- **Key length prefix is `uint16_t`** in `EloqKey::Serialize` (`eloqkv_key.h:227,243` — "represents key lengths up to 65535") while `MAX_KEY_SIZE` is 32 MB on non-EloqStore builds (`src/redis_service.cpp:198`). Keys between 64 KB and 32 MB would truncate their serialized length prefix; KV storage (`KVSerialize`, raw bytes) is unaffected. Flagged as a latent inconsistency (not observed failing; inference).
